...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/sfdigihw5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    new_prior = StringField("What is the new priority of the ToDo list item?",validators=[Required()])
    submit = SubmitField("Update")
...
def update(item):
    ...
    if form.validate_on_submit():
        newp = form.new_prior.data
        newp = str(newp)
        it = TodoItem.query.filter_by(description=item).first()
        it.priority = newp
        db.session.commit()
        flash("Updated priority of the " + newp)
        return redirect(url_for("all_lists"))
